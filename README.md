...
输入绘文字，是以 `/em` 进入绘文字状态，然后输入 shortcode。
此 shortcode 是根据我的个人喜好而决定的，并且不包含所有的绘文字。
主要目的是为了方便我日常使用中输入。若要完整的绘文字，可以用其它的工具，如 `rofimoji` 之类的。
举例：先输入 `/em`，紧接着输入 `star`，便可看见 ⭐。
内容请常考 `emoji_en.dict.yaml`。
